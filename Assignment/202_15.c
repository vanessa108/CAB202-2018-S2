...
//gcc 202_15.c -std=gnu99 -Wall -Werror -Ic:/cab202/ZDK -Lc:/cab202/ZDK -lZDK -lncurses -o 202
...
block_parameters_t block_parameters(void){
  ...
  if(((b_param.row_height * b_param.num_rows)+ DISPLAY_HEIGHT + HERO_HEIGHT) >= screen_h - 1) {
...
void update_treasure(int key){
  ...
  // while (toggle!= false) {
  //   sprite_set_image(treasure, treasure_2);
  //   //timer_pause(100);
  //   sprite_set_image(treasure, treasure_1);
  // }
...
void treasure_animation(){ 
  treasure_delay++;
  if (treasure_delay % 20 == 0 ) {
    if (image1 == true) {
      sprite_set_image(treasure, treasure_1);
    } else {
      sprite_set_image(treasure, treasure_2);
...
void hero_move(int key)
{
  ...
  else if (key == 'd' && x <(screen_w-6) && isStand) {
    sprite_move(hero, 1, 0);
