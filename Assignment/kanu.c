...
void screen_pos_write (screen_pos_t scr_pos) {
    //  (Task 3.1) Print the horizontal and vertical position of SCREEN_POS_VAR
...
int screen_pos_dist(screen_pos_t scr_pos_1, screen_pos_t scr_pos_2) {
    ...
    int x_distance = scr_pos_2.x_pos - scr_pos_1.x_pos;
    int y_distance = scr_pos_2.y_pos - scr_pos_1.y_pos;

    if (scr_pos_2.x_pos < scr_pos_1.x_pos) {
        x_distance *= -1;
    }

    if (scr_pos_2.y_pos < scr_pos_1.y_pos) {
        y_distance *= -1;
    }
...
//      this is so.
 ...
 screen_pos_t * screen_pos_nearest(screen_pos_t query_pos, screen_pos_t positions[], int num_positions) {
    ...
    int min_dist = screen_pos_dist(query_pos, positions[0]);
