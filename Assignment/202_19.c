...
//gcc 202_19.c -std=gnu99 -Wall -Werror -Ic:/cab202/ZDK -Lc:/cab202/ZDK -lZDK -lncurses -o 202
...
void hero_move(int key) {
  ...
  //hero moves left when a is pressed and on a block
    ...
    sprite_turn_to(hero, -0.5, 0);
    sprite_step(hero);
    sprite_set_image(hero, hero_left);
    //previous_key = 'a';
  }
  //hero moves right when d is pressed and on a block
  ...
  else if (key == 'a' && x > 0 && !isStand && previous_key != 'a'){
    hero->dx += -JUMP_DX;
    sprite_step(hero);
    previous_key = 'a';
  }
  else if (key == 'd' && x <(screen_width()-6) && !isStand && previous_key != 'd') {
    hero->dx += JUMP_DX;
    sprite_step(hero);
    previous_key = 'd';
  } 
  // else if (key == 'a' && !isStand && previous_key != 'a') {
  //   hero->dx += -JUMP_DX;
  //   sprite_step(hero);
  //   previous_key = 'a';
  // }
  // else if (key == 'd' && !isStand && previous_key != 'd') {
  //   hero->dx += JUMP_DX;
  //   sprite_step(hero);
  //   previous_key = 'd'
  // }
